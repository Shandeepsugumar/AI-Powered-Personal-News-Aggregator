...
export default function App() {
  ...
  const fetchArchivesList = async (token) => {
    if (!token) return;
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || "https://ai-powered-personal-news-aggregator.onrender.com"}` + ""}/api/newspaper/archives`, {
  ...
  const loadUserNewspaperAndArchives = async (token) => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || "https://ai-powered-personal-news-aggregator.onrender.com"}` + ""}/api/newspaper/latest`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  useEffect(() => {
    ...
    if (parsedUser?.token) {
      fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || "https://ai-powered-personal-news-aggregator.onrender.com"}` + ""}/api/sources`, {
  ...
  const handleSelectArchiveDate = async (dateStr) => {
    ...
    if (user?.token) {
      try {
        setIsLoading(true);
        const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || "https://ai-powered-personal-news-aggregator.onrender.com"}` + ""}/api/newspaper/archive?date=${encodeURIComponent(normalizedDate)}`, {
  ...
  const handleSelectEdition = async (editionId) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || "https://ai-powered-personal-news-aggregator.onrender.com"}` + ""}/api/newspaper/edition/${editionId}`, {
        headers: { 'Authorization': `Bearer ${user.token}` }
      });
      if (res.ok) {
        const edData = await res.json();
