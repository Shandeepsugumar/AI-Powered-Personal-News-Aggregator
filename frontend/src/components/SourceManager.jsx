...
export default function SourceManager({ isOpen, onClose, token, sources = [], onUpdateSources }) {
  ...
  useEffect(() => {
    const loadUserSources = async () => {
      ...
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || "https://ai-powered-personal-news-aggregator.onrender.com"}` + ""}/api/sources`, {
  ...
  const handleToggleSource = async (id) => {
    ...
    if (authToken && !String(id).startsWith('src_')) {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || "https://ai-powered-personal-news-aggregator.onrender.com"}` + ""}/api/sources/${id}/toggle`, {
          method: 'PATCH',
          headers: { 'Authorization': `Bearer ${authToken}` }
        });
        if (res.ok) {
  ...
  const handleDeleteSource = async (id) => {
    ...
    if (authToken && !String(id).startsWith('src_')) {
      try {
        await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || "https://ai-powered-personal-news-aggregator.onrender.com"}` + ""}/api/sources/${id}`, {
          method: 'DELETE',
          headers: { 'Authorization': `Bearer ${authToken}` }
        });
      } catch (err) {
