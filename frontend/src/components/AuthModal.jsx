...
export default function AuthModal({ isOpen, onClose, onAuthSuccess, user }) {
  ...
  const handleQuickDemo = async () => {
    ...
    try {
      // 1. Try login first
      let response = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || "https://ai-powered-personal-news-aggregator.onrender.com"}` + ""}/api/auth/login`, {
        method: 'POST',
      ...
      if (!response.ok) {
        response = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || "https://ai-powered-personal-news-aggregator.onrender.com"}` + ""}/api/auth/register`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleSubmit = async (e) => {
    ...
    const endpoint = isRegister ? `${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || "https://ai-powered-personal-news-aggregator.onrender.com"}` + ""}/api/auth/register` : `${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || "https://ai-powered-personal-news-aggregator.onrender.com"}` + ""}/api/auth/login`;
